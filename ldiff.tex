...
%DIF DEL ../oldonward/onward.tex   Wed Jun 25 12:41:29 2025
%DIF ADD onward.tex                Thu Jun 12 12:37:43 2025
...
I (read: ``my lab'') don't really do human subjects; I don't ever want \DIFdelbegin \DIFdel{do
}\DIFdelend \DIFaddbegin \DIFadd{to
}\DIFaddend write IRB paperwork.  I do work with \DIFdelbegin \DIFdel{people }\DIFdelend \DIFaddbegin \emph{\DIFadd{other people}} \DIFaddend who do human subjects,
...
paperwork and still make this a \DIFdelbegin \DIFdel{``real '' }\DIFdelend \DIFaddbegin \DIFadd{real research }\DIFaddend paper.

But I don't have a huge \DIFaddbegin \DIFadd{enough }\DIFaddend committment to Test
...
to do things in a TDD way, which is \DIFdelbegin \DIFdel{suggestive}\DIFdelend \DIFaddbegin \DIFadd{worrying}\DIFaddend .  So it just doesn't
seem worth it to commit to a serious Research Project, likely spanning
over a year, \DIFdelbegin \DIFdel{to this idea}\DIFdelend \DIFaddbegin \DIFadd{just to explore this idea (I have lots of other ideas
that I think are equally or more interesting, that I also don't have
time to make happen)}\DIFaddend .
...
\DIFdelbegin \DIFdel{implmentations }\DIFdelend \DIFaddbegin \DIFadd{implementations }\DIFaddend of test generation ideas I think might be interesting
...
\DIFaddbegin \DIFadd{Still, I didn't }\emph{\DIFadd{want}} \DIFadd{to do that; this idea has been bugging me
...
of it?  I don't particularly }\emph{\DIFadd{believe}} \DIFadd{in TDD (but I also don't hate
...
\DIFaddend \subsection{Daniel Jackson Made Me Do It}
...
more appropriate to UI-heavy software \DIFdelbegin \DIFdel{for ``normal people'' }\DIFdelend than the
...
compilers) I
\DIFdelbegin \DIFdel{like }\DIFdelend \DIFaddbegin \DIFadd{tend }\DIFaddend to test.  The real value of the book was, for me, twofold:  first, the
long long set of \DIFdelbegin \DIFdel{``notes '' }\DIFdelend \DIFaddbegin \DIFadd{notes }\DIFaddend after the main text is a delightful random
...
at least;
obviously \DIFdelbegin \DIFdel{almost every }\DIFdelend \DIFaddbegin \DIFadd{many a }\DIFaddend research project in SE is \DIFdelbegin \DIFdel{a joke }\DIFdelend \DIFaddbegin \DIFadd{not }\emph{\DIFadd{that}} \DIFadd{much work }\DIFaddend compared to
...
managers \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend an army of graduate students, but \DIFaddbegin \DIFadd{prefer instead to be
}\DIFaddend less-productive/more-hands-on (here's looking at myself in a mirror)\DIFaddbegin \DIFadd{,
and have many fewer meetings and Slack conversations}\DIFaddend .
...
make work in more \DIFdelbegin \DIFdel{``technical '' }\DIFdelend \DIFaddbegin \DIFadd{technical }\DIFaddend parts of SE, such as software testing,
...
\DIFdelbegin \DIFdel{``ideas '' }\DIFdelend \DIFaddbegin \DIFadd{ideas }\DIFaddend than a narrower scientific claim.

Now, many researchers take ideas that don't \DIFdelbegin \DIFdel{satsfy }\DIFdelend \DIFaddbegin \DIFadd{satisfy }\DIFaddend the current rules
...
essay\DIFaddbegin \footnote{\DIFadd{This is a joke!  The related statement that an essay is
  just a research paper missing experimental evidence is, of course,
  even less true; an essay is a ``jackdaw, a magpie, a
  raven''~\mbox{%DIFAUXCMD
\cite{doyleessay}}\hspace{0pt}%DIFAUXCMD
, and while some good research papers contain the germs of
  an essay, the two forms are generally distinct, not that you would
  be so sure of that from the example at hand, the essay you are reading.}}\DIFaddend .  The reader is free to try the idea out, with no guarantee that
...
is far from \emph{sufficient}\DIFdelbegin \DIFdel{.  Now, the most }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Discontents,CleanCode}}\hspace{0pt}%DIFAUXCMD
.  Admittedly, the }\emph{\DIFadd{most}} \DIFaddend glaring risk of code
...
The problem is that checking one behavior of a \DIFdelbegin \DIFdel{``piece '' }\DIFdelend \DIFaddbegin \DIFadd{small piece }\DIFaddend of code
...
undesirable, and that \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend satisfies more esoteric conditions \DIFdelbegin \DIFdel{``}\DIFdelend off the
path \DIFdelbegin \DIFdel{'' }\DIFdelend of developing functionality: e.g., that it fails in the \DIFdelbegin \DIFdel{``right '' }\DIFdelend \DIFaddbegin \DIFadd{right }\DIFaddend way when called with
invalid inputs.  \DIFaddbegin \DIFadd{Later, we'll see that this corresponds to knowing not
...
Code coverage is \DIFaddbegin \DIFadd{indeed }\DIFaddend guaranteed to be high, but the implied syllogism that
\DIFdelbegin \DIFdel{therefore }\DIFdelend \DIFaddbegin \emph{\DIFadd{therefore}} \DIFaddend this goal will be met \DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{(which I think much TDD
...
\DIFaddend However, code coverage is not the
...
synthetic bugs, and seeing if they are \DIFdelbegin \DIFdel{indeed }\DIFdelend detected.  In mutation
...
TDD \DIFdelbegin \DIFdel{probably, by construction , }\DIFdelend \DIFaddbegin \DIFadd{by construction }\DIFaddend ensures high enough coverage that
...
Figure~\ref{fig:mdd} \DIFaddbegin \DIFadd{(I omit the step of coming up with a new
behavior, because while this is essential it is }\emph{\DIFadd{outside the
  loop}}\DIFadd{, and I want to emphasize the change in the }\emph{\DIFadd{loop}}\DIFadd{)}\DIFaddend .
...
\DIFdelbegin \DIFdel{Then }\DIFdelend \DIFaddbegin \DIFadd{The code is organized into versions I extracted from the blog
post, numbered }{\tt \DIFadd{v1}} \DIFadd{through }{\tt \DIFadd{v6}}\DIFadd{.  It's a good idea to look at
...
lay of the land.
    ...
    \DIFadd{Then, to try out MDD, }\DIFaddend go into any directory, \DIFdelbegin \DIFdel{say }\DIFdelend \DIFaddbegin \DIFadd{e.g. }\DIFaddend {\tt v3}, and type:
...
def test\_remove\_pattern\_type\_int():
  ...
  \DIFdelbegin \DIFdel{``respectable '' }\DIFdelend \DIFaddbegin \DIFadd{respectable }\DIFaddend demonstration of TDD?
...
not killed all indicate these exact problems.  \DIFaddbegin \DIFadd{Is looking at ten
...
are an eager graduate student dying to turn this sketchy idea into a \DIFdelbegin \DIFdel{``real '' }\DIFdelend \DIFaddbegin \DIFadd{real }\DIFaddend paper)!

\DIFaddbegin
\section{The Mighty River: A Digression}

\DIFadd{This is an essay (thus a jackdaw, magpie, raven~\mbox{%DIFAUXCMD
\cite{doyleessay}}\hspace{0pt}%DIFAUXCMD
), not a research paper, so we can turn our minds aside
...
\DIFadd{Mark Twain's }\emph{\DIFadd{Life on the Mississippi}} \DIFadd{is one of the best books
ever written.  And one thing it makes you }\emph{\DIFadd{see}}\DIFadd{, that you may
...
every time young Twain thinks, cockily, that he }\emph{\DIFadd{has}} \DIFadd{done this
impossible thing, the irascible expert pilot Bixby tells him something
else insanely difficult he has to know:
}

\begin{quote}
\DIFadd{``Then I've got to work and learn just as much more river as I already
know.''
}

\DIFadd{``Just about twice as much more, as near as you can come at it.''
}

\DIFadd{``Well, one lives to find out.  I think I was a fool when I went into
this business.''
}

\DIFadd{``Yes, that is true.  And you are yet.  But you'll not be when you've
learned it.''
}

\DIFadd{``Ah, I never can learn it.''
}

\DIFadd{``I will see that you DO.''
}\end{quote}

\DIFadd{This was written when the river had no lights or bouys, and we may
...
\begin{figure}
  ...
  \caption{\DIFaddFL{Cartoon by Bradley, cartoonist of the Chicago Daily News, 1917}}
  \label{fig:twain}
  \end{figure}

\DIFadd{I think the change in point of view from code coverage (handled by
...
to know }\emph{\DIFadd{this}} \DIFadd{as well (how to read the river's dynamics from
...
rivers and larger boats await}\footnote{\DIFadd{One great advantage of this
  ...
  turn over to coding assistants.}}\DIFadd{.
}

\DIFadd{Coverage (TDD's forte; you not only have good coverage, but you have
...
of the river as it runs as well; mutation is a }\emph{\DIFadd{text-driven}} \DIFadd{form
...
\DIFaddend \section{A Bigger Picture}

If (it's a big if) MDD is a useful idea, I think it's a minor example
of where we get many nice ideas:  cross-overs.  I don't \DIFdelbegin \DIFdel{``do '' }\DIFdelend \DIFaddbegin \DIFadd{do }\DIFaddend TDD and
I think people who are interested in TDD in the research world are
mostly (I would guess, I don't read much TDD literature) ``process \DIFaddbegin \DIFadd{people}\DIFaddend ''
people.  I think \DIFdelbegin \DIFdel{``process '' }\DIFdelend \DIFaddbegin \DIFadd{process }\DIFaddend people who care about TDD probably think
a lot about testing, but don't think very deeply about code coverage
metrics (beyond the basic ``let's see what ran'') or the \DIFdelbegin \DIFdel{``oracle
problem''}\DIFdelend \DIFaddbegin \DIFadd{oracle
problem}\DIFaddend ~\cite{staats2011programs,oracleMcMinn} because these ideas are mostly siloed into the ``tools and
...
agile/XP/any of that \DIFdelbegin \DIFdel{``stuff.  ''  }\DIFdelend \DIFaddbegin \DIFadd{stuff.  }\DIFaddend I didn't know at the time Grenning
